...
action :change_cluster_node_type do
  ...
  var_cluster_node_type = parse_cluster_nodes_string(new_resource.cluster_nodes).select { |node| node['name'] == var_node_name || "'#{node['name']}'" == var_node_name }.first['type'] # ~FC039
